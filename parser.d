...
class Parse{
	...
	void SyntaxError(string err, string errtype="syntax", int lineno=0){
		writeln("ERROR: ", errtype, ": ", err);
		writeln("Traceback: " ~ this.file ~"\n");
		...
		if (exists(this.file)){
			...
			for(int i = 0; i < this.tok.loc + to!string(this.tok.loc).length; i++){
				write(' ');
			}
	...
	Node objectdata(){
		...
		while (this.end && this.tok.type != "}") {
			if (find("NL,", this.tok.type).length) {
				...
				if (!find("IDSTRNUM", this.tok.type)){
					this.SyntaxError("Invalid key for dict value");
				}
				...
				if (this.tok.type != ":"){
					this.SyntaxError("Expected token ':' not '"~this.tok.value~"'.");
				}
	...
	Node eval(string end){
		...
		if (!find(end, this.tok.type).length){
			if (find("STR", this.tok.type).length){
				this.SyntaxError("unexpected token ''" ~ this.tok.value ~ "'' in expression.");
			...
			while (this.end && this.tok.type != ")"){
				if (this.tok.type == "ID"){
					...
					if (this.tok.type == "="){
						...
						this.SyntaxError("Not accepted to write unassigned parameters after defaults have been set.");
					}

				} else if (!find("NL,", this.tok.type).length){
					this.SyntaxError("Found invalid token in function parameters.");
	...
	Token[] getCode(){
		...
		if (this.tok.type == "{"){
			...
			if (!this.end){
				this.SyntaxError("missing token '}' to close code block.", "indent", lineno);
			}
			this.next();
	...
	void parse_identifier(){
		...
		if (this.tok.type == "="){
			...
			this.next();
			this.ast ~= new VarNode(id, new NullNode());
			...
			this.SyntaxError("unexpected token '" ~ this.tok.value ~ "' after identifier.");
		}
	}
	...
	void parse_class(){
		...
		if (this.tok.type != "ID"){
			this.SyntaxError("Expected an Id for the ClName not token '" ~ this.tok.value ~ "'");
		}
	...
	void parse_for(){
		...
		if (this.tok.type != "IN"){
			this.SyntaxError("expected 'in' but given token '" ~ this.tok.value ~"'.");
		}
	...
	void parse_switch(){
		...
		if (this.tok.type == "ARR"){
			...
			if (this.tok.type == "BREAK"){
				...
				this.SyntaxError("Expected break token...");
			}
		}
	...
	void parse_import(){
		...
		if (this.tok.type == "STR"){
			while (this.tok.type != "NL" && this.end){
				if (this.tok.type == "STR" || this.tok.type == "ID"){
					mods~= this.tok.value;

				} else if (this.tok.type != ","){
					this.SyntaxError("Invalid syntax in import statement.");
				}
				this.next();
			}

			this.ast ~= new IncludeNode(mods);
			...
			this.ast ~= new ImportNode(data, path);
		}
	}
	...
	void parse_try(){
		...
		if (this.tok.type == "CATCH"){
			...
			if (this.tok.type != "ID"){
				this.SyntaxError("Expected ID token to store error.");
			}
